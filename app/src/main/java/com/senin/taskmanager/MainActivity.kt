        ...
        setContent {
            MaterialTheme {
                ...
                NavHost(nav, startDestination = "calendar") {
                    composable("calendar") {
                        CalendarScreen(
                            onAddTask = { date -> nav.navigate("add/$date") },
                            onSettings = { nav.navigate("settings") }
                        )
                    }
                    composable("add/{date}") { back ->
                        val dateStr = back.arguments?.getString("date") ?: LocalDate.now().toString()
                        AddTaskScreen(initialDate = LocalDate.parse(dateStr), onBack = { nav.popBackStack() })
                    }
                    composable("settings") {
