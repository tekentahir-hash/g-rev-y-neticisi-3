...
fun scheduleNotification(context: Context, taskId: Int, title: String, date: String, time: String) {
    ...
    val triggerAt = LocalDateTime.of(LocalDate.parse(date), LocalTime.of(hour, min))
        .atZone(ZoneId.systemDefault()).toInstant().toEpochMilli()
    if (triggerAt <= System.currentTimeMillis()) return

    val intent = Intent(context, NotificationReceiver::class.java).apply {
        putExtra("task_id", taskId); putExtra("task_title", title)
    }
    val pi = PendingIntent.getBroadcast(context, taskId, intent,
...
class NotificationReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        ...
        val pi = PendingIntent.getActivity(context, taskId, Intent(context, MainActivity::class.java),
            PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
        ...
        (context.getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager).notify(taskId, notification)
    }
}
