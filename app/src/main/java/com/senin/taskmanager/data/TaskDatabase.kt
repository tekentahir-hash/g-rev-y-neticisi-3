...
interface TaskDao {
    ...
    // Ayarlar: her groupId'den sadece 1 temsilci görev (tekrarlı listesi)
    @Query("""
        SELECT * FROM tasks 
        WHERE isDeleted = 0 AND frequency != 'ONCE' AND groupId != ''
        GROUP BY groupId
        ORDER BY frequency ASC, title ASC
    """)
