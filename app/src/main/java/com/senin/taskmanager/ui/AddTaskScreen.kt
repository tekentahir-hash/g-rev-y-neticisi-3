...
fun AddTaskScreen(
    ...
    var monthDay by remember { mutableStateOf<Int?>(null) }
    ...
    if (showTimePicker) {
        val timeState = rememberTimePickerState(initialHour = timeHour ?: 9, initialMinute = timeMin ?: 0, is24Hour = true)
        AlertDialog(
            onDismissRequest = { showTimePicker = false },
            confirmButton = { TextButton(onClick = { timeHour = timeState.hour; timeMin = timeState.minute; showTimePicker = false }) { Text("Tamam") } },
    ...
    if (showDatePicker) {
        val dateState = rememberDatePickerState(initialSelectedDateMillis = dueDate.toEpochDay() * 86400000L)
        DatePickerDialog(
            onDismissRequest = { showDatePicker = false },
            confirmButton = {
                TextButton(onClick = {
                    dateState.selectedDateMillis?.let { dueDate = LocalDate.ofEpochDay(it / 86400000L) }
        ...
        Column(
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                OutlinedButton(onClick = { showTimePicker = true }) {
                    Text(if (timeHour != null) "🕐 %02d:%02d".format(timeHour, timeMin) else "Saat seç")
                }
                if (timeHour != null) {
                    TextButton(onClick = { timeHour = null; timeMin = null }) { Text("Kaldır") }
                }
            }
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(4.dp)) {
                    dayLabels.forEach { (num, label) ->
                        FilterChip(
                            ...
                            onClick = { selectedDays = if (num in selectedDays) selectedDays - num else selectedDays + num },
            ...
            if (freq == TaskFrequency.WEEKLY) {
                Text("Her ${getDayName(dueDate.dayOfWeek.value)} tekrarlanacak",
                    style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.primary)
            }

            if (freq == TaskFrequency.MONTHLY) {
                Text("Her ayın ${dueDate.dayOfMonth}. günü tekrarlanacak",
                    style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.primary)
            }
            ...
            Button(
                onClick = {
                    if (title.isNotBlank()) {
                        ...
                        val days = if (freq == TaskFrequency.SPECIFIC_DAYS) selectedDays.toList() else emptyList()
                        viewModel.addTask(title, desc, freq, prio, dueDate, timeStr, days, dueDate.dayOfMonth)
                        onBack()
                    }
                },
